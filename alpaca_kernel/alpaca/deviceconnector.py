...
from tornado import websocket  # the old non async one
...
def guessserialport():
    ...
    lp.sort(key=lambda X: (X.hwid == "n/a",
                           X.device))  # n/a could be good evidence that the port is non-existent
                ...
                if wsresbufferI >= len(wsresbuffer):
                    ...
                    if r:
                        ...
                        if type(wsresbuffer) == str:
                            wsresbuffer = wsresbuffer.encode(
...
class DeviceConnector:
    ...
    def workingserialreadall(
            self):  # usually used to clear the incoming buffer, results are printed out rather than used
        ...
        if self.workingwebsocket:
            res = []
            while True:
                r, w, e = select.select([self.workingwebsocket], [], [],
    ...
    def disconnect(self, raw=False, verbose=False):
        if not raw:
            self.exitpastemode(
                verbose)  # this doesn't seem to do any good (paste mode is left on disconnect anyway)
    ...
    def serialconnect(self, portname, baudrate, verbose):
        ...
        if type(portname) is int:
            ...
            if possibleports:
                ...
                portname = (
                    "COM4" if sys.platform == "win32" else "/dev/ttyUSB0")

        self.sresSYS(
            "Connecting to --port={} --baud={} ".format(portname, baudrate))
        try:
            self.workingserial = serial.Serial(portname, baudrate,
    ...
    def websocketconnect(self, websocketurl):
        ...
        try:
            ...
            self.sres(
                "Websocket Timeout after 5 seconds {}\n".format(websocketurl))
    ...
    def receivestream(self, bseekokay, isplotting=0, bwarnokaypriors=True,
                      b5secondtimeout=False, bfetchfilecapture_nchunks=0):
        n04count = 0
        brebootdetected = False
        res = []
        for j in range(2):  # for restarting the chunking when interrupted
            if self.workingserialchunk is None:
                self.workingserialchunk = yieldserialchunk(
                    self.workingserial or self.workingsocket or self.workingwebsocket)
            ...
            for i, rline in enumerate(self.workingserialchunk):
                ...
                if bseekokay and bwarnokaypriors and (rline != b'OK') and (
                        rline != b'>') and rline.strip():
                    self.sres("\n[missing-OK]")
                ...
                # normal processing of the string of bytes that have come in
                    ...
                    if not wifimessageignore.match(ur):
                        if bfetchfilecapture_nchunks:
                            if res and res[-1][-2:] != "\r\n":
                                res[-1] = res[
                                              -1] + ur  # need to rejoin strings that have been split on the b"OK" string by the lexical parser
                            else:
                                res.append(ur)
                            if (i % 10) == 0 and bfetchfilecapture_nchunks > 0:
                                self.sres("%d%% fetched\n" % int(
                                    len(res) / bfetchfilecapture_nchunks * 100 + 0.5),
                                          clear_output=True)
                        else:
                            if not isplotting:
    ...
    def sendtofile(self, destinationfilename, bmkdir, bappend, bbinary, bquiet,
                   filecontents):
        ...
        if not bbinary:
            ...
            if maxlinelength > 250:
                self.sres(
                    "Line length {} exceeds maximum for line ascii files, try --binary\n".format(
                        maxlinelength), 31)
                return
        ...
        sswrite("O=open({}, '{}')\r\n".format(repr(destinationfilename),
                                              fmodifier).encode())
        ...
        if bbinary:
            ...
            for i in range(nchunks + 1):
                ...
                if (i % 10) == 9:
                    ...
                    self.receivestream(bseekokay=True)
                    if not bquiet:
                        self.sres("{}%, chunk {}".format(
                            int((i + 1) / (nchunks + 1) * 100), i + 1),
                                  clear_output=clear_output)
            self.sres(
                "Sent {} bytes in {} chunks to {}.\n".format(len(filecontents),
                                                             i + 1,
                                                             destinationfilename),
                clear_output=not bquiet)
            ...
            for i, line in enumerate(lines):
                ...
                if (i % linechunksize) == linechunksize - 1:
                    ...
                    if not bquiet:
                        self.sres("{}%, line {}\n".format(
                            int((i + 1) / (len(lines) + 1) * 100), i + 1),
                                  clear_output=clear_output)
            self.sres("Sent {} lines ({} bytes) to {}.\n".format(i + 1,
                                                                 len(filecontents),
                                                                 destinationfilename),
                      clear_output=(clear_output and not bquiet))
    ...
    def fetchfile(self, sourcefilename, bbinary, bquiet):
        ...
        if True:
            ...
            chunkres = self.receivestream(bseekokay=True,
                                          bfetchfilecapture_nchunks=-1)
            try:
            ...
            sswrite(b'\r\x04')
            chunks = self.receivestream(bseekokay=True,
                                        bfetchfilecapture_nchunks=nbytes // chunksize + 1)
            rres = []
            for ch in chunks:
                try:
    ...
    def enterpastemode(self,
                       verbose=True):  # I don't think we ever make a connection and it's still in paste mode (this is revoked on connection break, but I am trying to use exitpastemode to make it better)
        # now sort out connection situation
        if self.workingserial or self.workingwebsocket:
            sswrite = self.workingserial.write if self.workingserial else self.workingwebsocket.send

            time.sleep(
                0.2)  # try to give a moment to connect before issuing the Ctrl-C
            ...
            if verbose and l:
                ...
                self.sres(str(l))
            sswrite(
                b'1\x04')  # single character program to run so receivestream works
        else:
            self.workingsocket.write(
                b'1\x04')  # single character program "1" to run so receivestream works

        return self.receivestream(bseekokay=True, bwarnokaypriors=False,
